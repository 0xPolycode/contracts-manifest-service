...
function createManifestEvents(parsedBytecode) {
    return parsedBytecode.events.map(e => {
        ...
        console.log("parsedSignature: " + JSON.stringify(parsedSignature));
        const signatureTypes = e.substring(e.indexOf("(") + 1).replace(/\(/g, "tuple(");
...
function createManifestFunctions(parsedBytecode) {
    return parsedBytecode.functions.map(f => {
        ...
        console.log("parsedSignature: " + JSON.stringify(parsedSignature));
        const signatureTypes = f.substring(f.indexOf("(") + 1).replace(/\(/g, "tuple(");
